...
- **Multi-source / deep history — SEC EDGAR (not just yfinance)**: `three_statement_parser.parse_company_facts_series()` extracts the full **annual (10-K) + quarterly (10-Q)** statement series from SEC XBRL company-facts — ~19 years of history instead of yfinance's ~5 quarters (verified MMM 2007–2025). `scripts/backfill_edgar_full.py` runs it for all ~504 US equities (ticker→CIK via SEC `company_tickers.json`). `db.query_financial_statements` filters by `period_type` (QUARTERLY default; ANNUAL/ALL) so annuals coexist with quarterly without breaking TTM. `/api/financials` returns `annual_statements` (multi-year) + quarterly; the Financials page has a **Quarterly/Annual toggle**. Commit `3cbfd34`.
